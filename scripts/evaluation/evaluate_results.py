...
# ==========================================
DEFAULT_TARGET_DIRS = [
  "progressive_cgls/smoothed_sigma_zyx_0.2_0.2_0.2",
  "method_d_result/smoothed_sigma_zyx_0.2_0.2_0.2"
]

BASE_OUTPUT_DIR = "build/outputs"
...
def set_japanese_font():
  """
  OSにインストールされている日本語フォントを探して設定する
  """
  fonts = [
    "MS Gothic", "Hiragino Sans", "Yu Gothic", "Meiryo", 
  ...
  for f in fonts:
    if f in font_names:
      ...
      return
  print("Warning: Japanese font not found. Legends might be corrupted.")
...
def calculate_metrics(true_vol, pred_vol):
  ...
  return {
    "MSE_All": mse_all, "SSIM_All": ssim_all,
    "MSE_ROI": mse_roi, "SSIM_ROI": ssim_roi,
    "Scale_Factor": scale_factor
  }
...
# グラフ描画
# ==========================================
def plot_metrics(df, output_dir, max_iter, target_dirs_map):
  set_japanese_font() # 描画前に日本語フォントを設定
  plt.style.use('default')
  # フォント設定を維持するため再度呼び出し（一部のstyle適用でリセットされる場合があるため）
  set_japanese_font() 
  ...
  for col, title, fname in metrics_config:
    ...
    for method in methods:
      ...
      if not subset.empty:
        label_name = target_dirs_map.get(method, os.path.basename(method))
        plt.plot(subset['Iter_Int'], subset[col], marker='o', markersize=4, label=label_name, alpha=0.8)
    
    plt.title(title)
    plt.xlabel('Iteration (繰り返し回数)')
    plt.ylabel(col)
...
def resolve_method_dirs(method_names):
  ...
  for i, m in enumerate(method_names):
    ...
    if actual_path:
      ...
      if i == 0:
        label_map[actual_path] = "従来手法（CGLS）"
      elif i == 1:
        label_map[actual_path] = "提案手法（空間制約付きCGLS）"
      else:
        label_map[actual_path] = os.path.basename(actual_path)
    else:
...
def main():
  ...
  parser.add_argument("--methods", nargs='+', default=None, help="List of method directory names or paths")
  ...
  target_dirs, target_dirs_map = resolve_method_dirs(input_methods)
  ...
  for target_dir in target_dirs:
    print(f"\nProcessing: {target_dirs_map[target_dir]}")
  ...
  if all_results:
    ...
    plot_metrics(df, OUTPUT_DIR, args.max_iter, target_dirs_map)
