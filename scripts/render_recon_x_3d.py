...
def main():
  ...
  ap.add_argument("--vec", default="build/outputs/x_cgls_solution_vector.npy", help="推定密度ベクトル x のファイル")
  ap.add_argument("--grid3d", default="configs/grid3d.yml")
  ap.add_argument("--pairs", default="build/outputs/pairs.csv", help="元のpairs.csv")
  ap.add_argument("--low", type=float, default=None, help="青の等値面レベル")
  ap.add_argument("--hi",  type=float, default=None, help="赤の等値面レベル")
  ap.add_argument("--dpi", type=int, default=220)
  ap.add_argument("--out", default="ls_recon3d_render.png", help="出力PNGファイル名")
  
  # ★ 視点変更オプションを追加 ★
  ap.add_argument("--elev", type=float, default=22, help="仰角 (縦方向の角度, 0で真横)")
  ap.add_argument("--azim", type=float, default=-60, help="方位角 (横方向の回転)")
  ...
  # --- 2. データの読み込み ---
  try: x_vector = np.load(args.vec)
  except FileNotFoundError: print(f"[ERR] ファイルが見つかりません: {args.vec}"); return
  
  if x_vector.size != nx * ny * nz:
      print(f"[ERR] サイズ不一致"); return
  ...
  fig.suptitle(f"3D Recon (low≈{lvL:.3g}, high≈{lvH:.3g})\nView: elev={args.elev}, azim={args.azim}", y=0.94)

  out = out_path(Path(args.out).name)
  plt.tight_layout()
  plt.savefig(out, dpi=args.dpi, bbox_inches="tight")
  plt.close(fig)
  print(f"[OK] 画像を保存しました: {out}")
